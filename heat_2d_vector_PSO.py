...
# ax1.plot(rad.values[:, 2].tolist(), 'r')
# ax1.tick_params(axis='y', labelcolor=color)
#
# ax2 = ax1.twinx()
#
# color = 'tab:blue'
# ax2.set_ylabel('Ambient temperature [°C]', color=color)  # we already handled the x-label with ax1
# ax2.plot(Tamb_val_clean, color=color)
...
class heat_f2:
    ...
    def fitness(self, x):
        ...
        if (True):
            plt.savefig("ImagesPSO/{:02.0f}{}.png".format(f2, 'fitness'), format='png', dpi=300)
            print('Image ADDED!')
        else:
